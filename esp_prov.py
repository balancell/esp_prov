...
idf_path = os.path.abspath(os.getcwd())
os.environ['LOCAL_IDF_PATH'] = os.path.join(idf_path, 'idf_deps')
print(idf_path)
sys.path.insert(0, os.path.join(idf_path, 'idf_deps', 'components', 'protocomm', 'python'))
sys.path.insert(1, idf_path)
    ...
    try:
        ...
        if (sel_transport == 'softap'):
            ...
            nu_lookup = {'prov-session': 'ff51',
                         'prov-config': 'ff52', 'proto-ver': 'ff53'}
...
def scan_wifi_APs(sel_transport, tp, sec):
    ...
    try:
        ...
        stop_time = time.time()
        print('++++ Scan process executed in ' +
              str(stop_time - start_time) + ' sec')
...
def wait_wifi_connected(tp, sec):
    ...
    while True:
        ...
        if ret == 'connecting':
            ...
            retry -= 1
            print(
                'Waiting to poll status again (status %s, %d tries left)...' % (ret, retry))
        else:
    ...
    if args.ssid == '':
        ...
        while True:
            ...
            while True:
                try:
                    select = int(
                        binput('Select AP by number (0 to rescan) : '))
